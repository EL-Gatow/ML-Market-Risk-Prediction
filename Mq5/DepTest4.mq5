...
string BuildJSON()
{
   ...
   if(lowPrice == 0)
   {
      Print("Error: Gagal mengambil Low price");
      return "";
   }
   ...
   Print("[DEBUG] Low:", lowPrice, " RSI:", rsi[0], " MA:", ma[0], " MACD_Main:", macdMain[0], " MACD_Signal:", macdSignal[0], " ATR:", atr[0]);
   string json = StringFormat(
      "{\"Low\":%.5f,\"RSI_14\":%.5f,\"MA_20\":%.5f,\"MACD_Main\":%.5f,\"MACD_Signal\":%.5f,\"ATR_14\":%.5f}",
      SafeDouble(lowPrice), SafeDouble(rsi[0]), SafeDouble(ma[0]),
      SafeDouble(macdMain[0]), SafeDouble(macdSignal[0]), SafeDouble(atr[0])
   );
...
void OnTimer()
{
   // Tidak perlu mengirim data secara terpisah lagi
   // Karena data sudah disimpan saat ShouldEntry dipanggil
   
   // Cek apakah sudah waktunya untuk memeriksa kondisi entry
   if(TimeCurrent() - lastDataSent >= DataSendInterval)
   {
      // Panggil ShouldEntry() untuk mengirim data ke server
      // dan mendapatkan sinyal trading
      bool shouldEnter = ShouldEntry();
      Print("[TIMER] Sinyal entry: ", shouldEnter ? "Ya" : "Tidak");
