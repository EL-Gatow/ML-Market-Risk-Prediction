...
string BuildJSON()
{
   ...
   double highPrice = iHigh(_Symbol, PERIOD_M15, 0); // Tambahkan high price
   
   if(lowPrice == 0 || highPrice == 0)
   {
      Print("Error: Gagal mengambil Low/High price");
      return "";
   }
   ...
   Print("[DEBUG] Low:", lowPrice, " High:", highPrice, " RSI:", rsi[0], " MA:", ma[0], " MACD_Main:", macdMain[0], " MACD_Signal:", macdSignal[0], " ATR:", atr[0]);
   string json = StringFormat(
      "{\"Low\":%.5f,\"High\":%.5f,\"RSI_14\":%.5f,\"MA_20\":%.5f,\"MACD_Main\":%.5f,\"MACD_Signal\":%.5f,\"ATR_14\":%.5f}",
      SafeDouble(lowPrice), SafeDouble(highPrice), SafeDouble(rsi[0]), SafeDouble(ma[0]),
